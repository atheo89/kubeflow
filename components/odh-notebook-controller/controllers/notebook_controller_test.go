...
var _ = Describe("The Openshift Notebook controller", func() {
	...
	When("Checking ds-pipeline-config secret lifecycle", func() {
		...
		BeforeEach(func() {
			//Pass env to be visible on the test suite
			os.Setenv("SET_PIPELINE_SECRET", "true")
		...
		It("should create a ds-pipeline-config secret if a DSPA is present", func() {
			...
			Eventually(func() error {
				...
				if err != nil {
					fmt.Fprintf(GinkgoWriter, "ds-pipeline-config not found: %v\n", err)
				}
				return err
			...
			for key, val := range fetchedSecret.Data {
				fmt.Fprintf(GinkgoWriter, "🔍 Secret Key: %s\n", key)
				fmt.Fprintf(GinkgoWriter, "🔐 Value:\n%s\n", string(val))
			}
			fmt.Fprintln(GinkgoWriter, "🔗 Inspecting ownerReferences of ds-pipeline-config Secret:")
			if len(fetchedSecret.OwnerReferences) == 0 {
				fmt.Fprintln(GinkgoWriter, "⚠️  No ownerReferences found on ds-pipeline-config Secret!")
			} else {
				for _, ref := range fetchedSecret.OwnerReferences {
					fmt.Fprintf(GinkgoWriter, "- Kind: %s, Name: %s, UID: %s, Controller: %v\n",
			...
			_ = cli.Get(ctx, types.NamespacedName{Name: "dspa", Namespace: "default"}, dspa)
			fmt.Printf("Finalizers on DSPA: %v\n", dspa.Finalizers)
