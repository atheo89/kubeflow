...
func (r *OpenshiftNotebookReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Use UpdateImageTriggerAnnotation to update the workbench image
	// when the imagestream information is available via
	//annotation `notebooks.opendatahub.io/last-image-selection`
	err = r.UpdateImageTriggerAnnotation(notebook, ctx)
	if err != nil {
		return ctrl.Result{}, err
	}
...
// UpdateImageTriggerAnnotation update the trigger annotation to the notebook
func (r *OpenshiftNotebookReconciler) UpdateImageTriggerAnnotation(notebook *nbv1.Notebook, ctx context.Context) error {

	// Initialize logger format
	log := r.Log.WithValues("notebook", notebook.Name, "namespace", notebook.Namespace)
	var isTriggerPaused bool
